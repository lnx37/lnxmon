...
// GzipResponseWriter embeded io.Writer and http.ResponseWriter,
// both of them have method Write(),
// without overriding, the selector will never know the method Write() belongs to which one when it is invoked,
// thus the compiler complains this error.
...
// func (gzip_response GzipResponseWriter) WriteHeader(code int) {
// 	log.Println("code:", code)
// 	gzip_response.Header().Del("Content-Length")
// 	gzip_response.Header().Set("Content-Encoding", "gzip")
// 	gzip_response.ResponseWriter.WriteHeader(code)
// }
...
func SelectHost(db *sql.DB, id int64) map[string]interface{} {
	...
	var query string
	query = "SELECT code, hostname, ip, cpu_processors, mem_size, swap_size, project FROM t_host WHERE id=?"
	...
	err = row.Scan(&code, &hostname, &ip, &cpu_processors, &mem_size, &swap_size, &project)
	Throw(err)
...
func SelectHostMetric(db *sql.DB, host map[string]interface{}, offset int64, limit int64) map[string]interface{} {
	var err error
	...
	// host["xxx"] could be nil when there is no data
	project = host["project"].(string)
	code = host["code"].(string)
	mem_size = host["mem_size"].(int64)
	swap_size = host["swap_size"].(int64)
	...
	for rows.Next() {
...

		{
			...
			for _, field = range fields {
				...
				key_disk = fmt.Sprintf("disk_usage of %s (%sG)", mount_point, disk_total)
				key_inode = fmt.Sprintf("inode_usage of %s (%sG)", mount_point, disk_total)
	...
	mem_usage_array = append(mem_usage_array, generate_series(fmt.Sprintf("mem_usage (%dG)", mem_size), mem_used_array))
	mem_usage_array = append(mem_usage_array, generate_series(fmt.Sprintf("swap_usage (%dG)", swap_size), swap_used_array))
...
func Index(response http.ResponseWriter, request *http.Request) {
	...
	var host_metric map[string]interface{}
	host_metric = SelectHostMetric(db, host, offset2, limit2)
	...
	if HTML == "" {
		...
		HTML = strings.ReplaceAll(HTML, "there_is_a_backtick", "`")
...
func ReportHost(response http.ResponseWriter, request *http.Request) {
...

	{
		...
		query = `
			UPDATE t_host
			SET
				hostname=?, ip=?, os_type=?, architecture=?, kernel=?, cpu_processors=?,
	...
	if rows_affected == 0 {
		...
		query = `
			INSERT INTO t_host (
				code, hostname, ip, os_type, architecture, kernel, cpu_processors,
...
func main() {
	...
	http.HandleFunc("/", MakeGzipHandler(MakeHandler(Index)))
	http.HandleFunc("/index", MakeGzipHandler(MakeHandler(Index)))
	...
	http.HandleFunc("/api/get_host_metric", MakeGzipHandler(MakeHandler(GetHostMetric)))
